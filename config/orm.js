...
var orm = {
  ...
  insertOne: function(wine_name, callback){
    ...
    // Run MySQL Query
    connection.query('INSERT INTO wines SET ?', {
      wine_name: wine_name,
      drank: false,
      date: timestamp
    }, function (err, result) {
  ...
  // updateOne()
  updateOne: function(wineID, callback){

    // Run MySQL Query
    connection.query('UPDATE wines SET ? WHERE ?', [{drank: true}, {id: wineID}], function (err, result) {
        if (err) throw err;
        callback(result);
      });
